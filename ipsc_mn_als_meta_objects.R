...
# sbatch -N 1 -c 12 --mem=72G -t 72:00:00 --wrap="Rscript /camp/lab/luscomben/home/users/ziffo/projects/ipsc-mn-als-meta/scripts/ipsc_mn_als_meta/ipsc_mn_als_meta_objects.R" --mail-type=ALL,ARRAY_TASKS --mail-user=oliver.ziff@crick.ac.uk --job-name=DESeq2objects
...
# shared_path = here("/Volumes/lab-luscomben/home/users/ziffo/proj-luscombn-patani/working")myq
...
lee.metadata <- read_csv(here(shared_path,"public-data/ipsc-occular-spinal-mn-lee-2021/sample-details/samplesheet.csv")) %>% distinct(sample, .keep_all = TRUE) %>%
  mutate(database_dir = here(shared_path,"public-data/ipsc-occular-spinal-mn-lee-2021"), dataset = case_when(mutation == "ctrl" ~ "lee.CTRL", TRUE ~ "lee.ALS"), instrument = instrument_model, condition = case_when(mutation == "ctrl" ~ "ctrl", TRUE ~ "als")) %>% filter(location == "spinal") %>% left_join(select(lee.gene_tpm.male_counts, sample, gender))
...
rm(answerals.gene_tpm.male_counts, neurolincs.gene_tpm.male_counts, catanese.gene_tpm.male_counts, dafinca.c9orf72.gene_tpm.male_counts, dafinca.tardbp.gene_tpm.male_counts,  luisier.gene_tpm.male_counts, lee.gene_tpm.male_counts,#mitchell.gene_tpm.male_counts
   wang.gene_tpm.male_counts, kiskinis.gene_tpm.male_counts, sareen.gene_tpm.male_counts, sommer.gene_tpm.male_counts, sterneckert.gene_tpm.male_counts, kapeli.gene_tpm.male_counts, desantis.gene_tpm.male_counts, smith.gene_tpm.male_counts, bhinge.gene_tpm.male_counts, hawkins.gene_tpm.male_counts) 
...
# nygc_postmortem_spinal_cord.als_vs_ctrl.tissue$dds <- DESeqDataSetFromMatrix(countData = nygc_spinal_cord_rsem_counts.mat, colData = nygc_spinal_cord_metadata, design = ~ gender + library_prep + tissue + condition) %>% DESeq() # adjust for spinal cord site
