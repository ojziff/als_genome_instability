...
#source("/camp/lab/luscomben/home/users/ziffo/scripts/functions/all_functions_r_4.0.0.R")
...
mls
sar
sbatch -N 1 -c 8 --mem=40G -t 4:00:00 --wrap="Rscript /camp/lab/luscomben/home/users/ziffo/projects/ipsc-mn-als-meta/scripts/ipsc_mn_als_meta_objects.R" --mail-type=ALL,ARRAY_TASKS --mail-user=oliver.ziff@crick.ac.uk --job-name=DESeq2objects 
```
...
ipsc_mn_als_datasets.pcaData <- plotPCA(ipsc_mn_als_datasets$vsd, intgroup=c("sample", "condition", "replicate", "mutation", "sample", "dataset", "strandedness", "instrument", "total_spots", "total_size","read_count", "age", "disease_state", "tissue", "cell_type", "accession", "study_accession", "instrument_model", "instrument_platform","read_count","bases"), returnData=TRUE) %>% 
  mutate(instrument = case_when(dataset == "lee" & condition == "ctrl" ~ "Illumina NovaSeq 6000", TRUE ~ instrument), dataset = as.character(dataset), dataset = case_when(dataset %in% c("dafianca.c9orf72","dafianca.tardbp") ~ "dafianca", TRUE ~  dataset),
         DIV = as.factor(case_when(dataset %in% c("catanese","lee","luisier", "sareen","kapeli","smith","singapore") ~ 35, dataset == "dafianca" ~ 30, dataset == "wang" ~ 12, dataset == "melamed" ~ 28, dataset == "desantis" ~ 19, dataset == "moccia" ~ 30, dataset == "sterneckert" ~ 40)),
         instrument = gsub("Illumina ","",instrument), total_size = case_when(dataset == "lee" & condition == "ctrl" ~ read_count, dataset == "luisier" ~ bases, TRUE ~ total_size))
...
motor_neuron_markers = celltype.markers.marker$gene_name[celltype.markers.marker$gene_name %in% motor.neuron.markers]
...
celltype.markers.rnaseq.heatmap <- Heatmap(celltype.markers.marker.mat, 
          ...
          bottom_annotation = columnAnnotation(markers = anno_block(gp = gpar(fill = rep("white",13)), labels = c("Catanese", "Lee", "Dafianca", "Luisier", "Wang","Moccia","Sareen","Sterneckert","Kapeli","Melamed","DeSantis","Smith","Signapore"), labels_gp = gpar(fontsize = 6), labels_rot = 0)), column_split = c(rep(1,12),rep(2,9),rep(3,22),rep(4,6),rep(5,4),rep(6,5),rep(7,8),rep(8,6),rep(9,6),rep(10,4),rep(11,6),rep(12,5),rep(13,4)))
...
vsd_counts.celltype.marker <-  ipsc_mn_als_datasets$vsd.counts  %>% filter(gene_name %in% motor.neuron.markers) %>% select(-gene_id) %>% mutate_if(is.numeric, funs((. - 4.505))) #rowwise() %>%
 ...
 #        "motor.neuron fetal" = mean(c_across(starts_with("motor.neuron_fetal")), na.rm = TRUE), 
 #        "motor.neuron adult" = mean(c_across(starts_with("motor.neuron_adult_temporal")), na.rm = TRUE), 
 ...
 #  select(gene_name, Myeloid, Neuron, Oligodendrocyte, `motor.neuron adult`, `motor.neuron fetal`, `Bradley et al`, `Current study`, -gene_id) %>%
  # filter(rowMeans(across(where(is.numeric))) > 3.31) %>% # remove genes with no expression across samples
  # mutate_if(is.numeric, funs((. - 4.505)))  %>% # subtract from all values so start from 0 
  ...
  # transpose_tibble(old_rownames = "group", new_rownames = "gene_name") #%>% arrange(`Current study`)# transpose back: columns > rows, order by expression in our motor.neurons
...
motor.neuron.marker.heatmap <- Heatmap(vsd_counts.celltype.marker.mat, 
          ...
          left_annotation = rowAnnotation(markers = anno_block(gp = gpar(fill = c("white")), labels = c("Motor neuron markers"), labels_gp = gpar(fontsize = 10))),
          row_title = NULL,
          column_names_rot = 90, 
          column_title = NULL)
...
```{r}
# png("/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/figS2.heatmaps.merged.png",width=10,height=14,units="in",res=1200)
# celltype.markers.rnaseq.heatmap %v% differentiation.markers.rnaseq.heatmap %v% HOX.markers.rnaseq.heatmap
# dev.off()
...
figS2.merged <- ggarrange(
  ...
  nrow = 3, labels = c("a","b","c"),heights = c(2,1,0.5))
figS2.merged
ggsave(figS2.merged, filename = "/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/figS2.heatmaps.merged.png", height = 16, width = 12)
