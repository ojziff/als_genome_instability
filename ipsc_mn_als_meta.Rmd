...
ipsc_mn_als_progeny.p53weights %>% filter(gene_name == "MDM2")
```
...
## Non TDP-43 nuclei per sample

```{r}
neuronal_nuclei_tdp43_liu$vsd.counts = assay(neuronal_nuclei_tdp43_liu$vsd) %>% as_tibble(rownames = "gene_id") %>% left_join(gene2ens)
neuronal_nuclei_tdp43_liu.counts <- neuronal_nuclei_tdp43_liu$vsd.counts %>%
  dplyr::mutate_if(~ any(is.na(.x)), ~ if_else(is.na(.x),0,.x)) %>% select(-gene_id) %>% distinct(gene_name, .keep_all = TRUE) %>% column_to_rownames(var = "gene_name") %>% as.matrix()
neuronal_nuclei_tdp43_liu.sample_acts <- run_wmean(mat=neuronal_nuclei_tdp43_liu.counts, net=net_progeny, .source='source', .target='target', .mor='weight', times = 100, minsize = 5)
neuronal_nuclei_tdp43_liu.sample_acts

neuronal_nuclei_tdp43_liu.sample_acts %>%
  filter(statistic == 'norm_wmean', source == "p53") %>%
  mutate(group = case_when(grepl("neg",condition)~"TDP43\nnegative", TRUE~"TDP43\npositive")) %>%
  violin_plot(continuous = "score", color_by = "group", plot_stats = FALSE, plot_violin = FALSE, arrow_labels = FALSE, ylabel = "p53 activity score")

neuronal_nuclei_tdp43_liu.sample_acts_mat <- neuronal_nuclei_tdp43_liu.sample_acts %>%
...
# Scale per sample
neuronal_nuclei_tdp43_liu.sample_acts_mat <- scale(neuronal_nuclei_tdp43_liu.sample_acts_mat)
...
# Plot
Heatmap(neuronal_nuclei_tdp43_liu.sample_acts_mat, cluster_rows = FALSE) 
...
mean(ipsc_mn_als_datasets.starfusion.csv$age_at_sample_collection, na.rm = TRUE)  
...
```{r}
ipsc_mn_als_datasets.starfusion.mutation.fitg.forrest_plot = plot_summs(ipsc_mn_als_datasets.starfusion.sporadic.fitg, ipsc_mn_als_datasets.starfusion.c9orf72.fitg, ipsc_mn_als_datasets.starfusion.sod1.fitg, ipsc_mn_als_datasets.starfusion.tardbp.fitg, ipsc_mn_als_datasets.starfusion.fus.fitg,
           # model.names = c("Sporadic", "C9orf72","SOD1","TARDBP","FUS"),
           coefs = c("Sporadic"="genetic_groupSporadic","C9orf72"="genetic_groupC9orf72","SOD1"="genetic_groupSOD1","TARDBP"="genetic_groupTARDBP","FUS"="genetic_groupFUS"),
           plot.distributions = TRUE, rescale.distributions = TRUE, 
           colors = c("dodgerblue2","firebrick2","forestgreen","gold2","grey"), legend.title	= "",exp = FALSE,point.shape = FALSE, point.size = 1) + 
  coord_cartesian(xlim = c(-0.7,0.7)) + ggtitle("iPSMN") + 
...
nygc_spinal_cord.starfusion.mutation.fitg.forrest_plot = plot_summs(nygc_spinal_cord.starfusion.sporadic.fitg, nygc_spinal_cord.starfusion.c9orf72.fitg, nygc_spinal_cord.starfusion.sod1.fitg,
           # model.names = c("Sporadic","C9orf72","SOD1"),
           coefs = c("Sporadic"="genetic_groupSporadic","C9orf72"="genetic_groupC9orf72","SOD1"="genetic_groupSOD1"), plot.distributions = TRUE, rescale.distributions = TRUE, 
           colors = c("dodgerblue2","firebrick2","forestgreen","gold2","grey"), legend.title	= "",exp = FALSE,point.shape = FALSE, point.size = 1) + 
  coord_cartesian(xlim = c(-0.7,0.7)) + ggtitle("Postmortem") +
 theme_classic() + theme(legend.position = "none", axis.text = element_text(colour = "black", size = 8), axis.title = element_text(colour = "black", size = 9, face ="plain"), strip.text.y = element_text(colour = "black", size = 7), plot.title = element_text(hjust = 0.5, size = 9)) + guides(color=guide_legend(keywidth=0.1, keyheight=0.15, default.unit="inch")) +
  annotation_custom(grid::textGrob(label = paste0("increase in ALS"), y = unit(0.08, "npc"), x = unit(0.8, "npc"), gp = grid::gpar(fontsize = 8, fontface=1), rot = 0)) + 
  annotation_custom(grid::textGrob(label = paste0("increase in CTRL"), y = unit(0.08, "npc"), x = unit(0.2, "npc"), gp = grid::gpar(fontsize = 8, fontface=1), rot = 0)) + 
  annotation_custom(grid::linesGrob(arrow=arrow(type="open", ends="first", length=unit(3,"mm")), gp=gpar(col="black", lwd=1), x = c(0.95,0.65), y = unit(0.03, "npc"))) +
...
ggsave(ipsc_mn_als_datasets.gene_fusion_merged, filename = here(proj_path,"rnafusion/ipsc_mn_als_datasets.gene_fusion_merged.png"), height = 8, width = 10)
ggsave(ipsc_mn_als_datasets.gene_fusion_merged, filename = here(proj_path,"rnafusion/ipsc_mn_als_datasets.gene_fusion_merged.pdf"), height = 8, width = 10)
