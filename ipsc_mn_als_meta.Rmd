...
ipsc_mn_als_datasets.prefilt = readRDS(here(proj_path,"expression/deseq2/ipsc_mn_als_datasets.prefilt.rds"))
ipsc_mn_als_datasets.gender = readRDS(here(proj_path,"expression/deseq2/ipsc_mn_als_datasets.gender.rds"))
...
credentials::set_github_pat("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") 
# make github
library(gert)
...
```{r}
ipsc_mn_als_datasets.pcaData <- plotPCA(ipsc_mn_als_datasets$vsd, intgroup=c("sample", "dataset", "database_dir", "condition", "mutation", "dataset_sample", "strandedness", "onset_site", "study_accession", "DIV", "instrument","library_type"),
...
returnData=TRUE) %>% as_tibble() %>%
  ...
  left_join(select(ipsc_mn_als_datasets.metadata, dataset_sample, gender)) 
  

# multiqc stats
multiqc_general_stats_stat_paths = ipsc_mn_als_datasets.metadata %>% distinct(database_dir) %>% mutate(multiqc_general_stats_stat_paths = paste0(database_dir,"/nfcore/multiqc/star_salmon/multiqc_data/multiqc_general_stats.txt"), 
...
names(multiqc_general_stats.tsv) =  ipsc_mn_als_datasets.metadata %>% distinct(database_dir,.keep_all=TRUE) %>% pull(database_dir)
multiqc_general_stats <- map_dfr(multiqc_general_stats.tsv, bind_rows, .id = "database_dir") %>% clean_names()
multiqc_rseqc_read_distribution_paths = ipsc_mn_als_datasets.metadata %>% distinct(database_dir) %>% mutate(multiqc_rseqc_read_distribution_paths = paste0(database_dir,"/nfcore/multiqc/star_salmon/multiqc_data/multiqc_rseqc_read_distribution.txt"), 
...
names(multiqc_rseqc_read_distribution.tsv) =  ipsc_mn_als_datasets.metadata %>% distinct(database_dir,.keep_all=TRUE) %>% pull(database_dir)
multiqc_rseqc_read_distribution <- map_dfr(multiqc_rseqc_read_distribution.tsv, bind_rows, .id = "database_dir") %>% clean_names()
ipsc_mn_als_datasets.pcaData = ipsc_mn_als_datasets.pcaData %>% left_join(multiqc_general_stats) %>% left_join(multiqc_rseqc_read_distribution) # need unique sample names - If multiple dataset names within a single multiqc_general_stats.txt then will only join the first dataset name
...
ipsc_mn_als_datasets.pca_dataset <- ggplot(ipsc_mn_als_datasets.pcaData, aes(PC1, PC2, color=dataset)) + geom_point(size=1) + 
  ...
  # ggforce::geom_mark_ellipse(data = filter(ipsc_mn_als_datasets.pcaData, dataset %in% c("sommer", "neurolincs_diMN","bhinge","desantis", "wang","dafinca","kiskinis")),  aes(label = dataset), label.fontsize = 8, label.fontface = "plain", con.type = "straight", con.cap = 0.5, show.legend = FALSE)
...
# Colour by gender
ipsc_mn_als_datasets.pca_gender <- ggplot(ipsc_mn_als_datasets.pcaData, aes(PC1, PC2, color=gender)) + geom_point(size=1) + 
  theme_oz() + theme(legend.text=element_text(size=6), legend.title=element_text(size=7), legend.position = "top", legend.box = "vertical", legend.spacing = unit(0.01, 'cm')) + guides(color=guide_legend(keywidth=0.1, keyheight=0.1, default.unit="inch")) +
  xlab(paste0("PC1: ",round(100 * attr(ipsc_mn_als_datasets.pcaData, "percentVar"))[1],"% variance")) +  ylab(paste0("PC2: ",round(100 * attr(ipsc_mn_als_datasets.pcaData, "percentVar"))[2],"% variance")) 
ipsc_mn_als_datasets.pca_gender
...
ipsc_mn_als_datasets.pca_library_type <- ggplot(ipsc_mn_als_datasets.pcaData, aes(PC1, PC2, color=library_type)) + geom_point(size=1) + 
  theme_oz() + theme(legend.text=element_text(size=6), legend.title=element_text(size=7), legend.position = "top", legend.box = "vertical", legend.spacing = unit(0.01, 'cm')) + guides(color=guide_legend(keywidth=0.1, keyheight=0.1, default.unit="inch")) +
  xlab(paste0("PC1: ",round(100 * attr(ipsc_mn_als_datasets.pcaData, "percentVar"))[1],"% variance")) +  ylab(paste0("PC2: ",round(100 * attr(ipsc_mn_als_datasets.pcaData, "percentVar"))[2],"% variance"))+labs(colour="Library preparation")
...
ipsc_mn_als_datasets.pca_merged = ggarrange(
  ipsc_mn_als_datasets.pca_dataset, 
  ggarrange(ipsc_mn_als_datasets.pca_gender, ipsc_mn_als_datasets.pca_DIV, ncol = 2, labels = c("b","c")), 
  ggarrange(ipsc_mn_als_datasets.pca_als, ipsc_mn_als_datasets.pca_mutation, ncol = 2, labels = c("d","e")),
  ggarrange(ipsc_mn_als_datasets.pca_library_type, ipsc_mn_als_datasets.pca_instrument, ncol = 2, labels = c("f","g")),
...
ipsc_mn_als_datasets.pca_nfcore_qc_metrics.merged = ggarrange(
  ggarrange(ipsc_mn_als_datasets.pca_strandedness, ipsc_mn_als_datasets.pca_read_distribution, ncol = 2, labels = c("a","b")),
  ggarrange(ipsc_mn_als_datasets.pca_samtools_mqc_generalstats_samtools_raw_total_sequences,  ipsc_mn_als_datasets.pca_picard_mqc_generalstats_picard_percent_duplication, ncol = 2, labels = c("c","d")),
  ggarrange(ipsc_mn_als_datasets.pca_biotype_counts_mqc_generalstats_biotype_counts_percent_r_rna, ipsc_mn_als_datasets.pca_quali_map_mqc_generalstats_qualimap_5_3_bias, ncol = 2, labels = c("e","f")),
  ggarrange(ipsc_mn_als_datasets.pca_star_mqc_generalstats_star_uniquely_mapped_percent, ipsc_mn_als_datasets.pca_samtools_mqc_generalstats_samtools_reads_mapped, ncol = 2, labels = c("g","h")), 
...
ipsc_mn_als_datasets.vsd.loadings.scatter = ggplot(ipsc_mn_als_datasets.vsd.loadings, aes(x = PC1, y = PC2 )) + 
  ...
  geom_text_repel(data = filter(ipsc_mn_als_datasets.vsd.loadings, abs(PC1) > 0.04, abs(PC2) > 0.04, grepl("^SNOR|RN7SK|RMRP|RPPH1",gene_name)), aes(x = PC1, y = PC2, label = gene_name), colour = "firebrick2", size = 2.3) +
    # geom_text_repel(data = filter(ipsc_mn_als_datasets.vsd.loadings, abs(PC1) > 0.04, abs(PC2) > 0.04, gene_name %in% nonpolyadenylated_genes), aes(x = PC1, y = PC2, label = gene_name), colour = "firebrick2", size = 2.3) +
  ...
  annotate("text", x = 0.04, y = 0.06, label = "PC1+ PC2+",size = 3, color = "black") + annotate("text", x = -0.07, y = -0.14, label = "PC1- PC2-", size = 3, color = "firebrick2") + 
  annotate("text", x = 0.04, y = -0.14, label = "PC1+ PC2-", size = 3, color = "black") + annotate("text", x = -0.07, y = 0.06, label = "PC1- PC2+", size = 3, color = "black")
...
ipsc_mn_als_datasets.hc$labels <- gsub("dafinca.tardbp", "dafinca", ipsc_mn_als_datasets.hc$labels) %>% gsub("dafinca.c9orf72", "dafinca", .) %>% gsub("mn_d25_", "", .)
...
# ipsc_mn_als_datasets.hc.single$labels <- gsub("dafinca.tardbp", "dafinca", ipsc_mn_als_datasets.hc.single$labels) %>% gsub("dafinca.c9orf72", "dafinca", .) %>% gsub("mn_d25_", "", .) %>% gsub("REP", "", .) %>% gsub("lee_spinal", "HB9::GFP",.) %>% gsub("lee_ocular", "PHOX2B::GFP", .)
...
```{r}
rownames(colData(ipsc_mn_als_datasets$vsd)) = rownames(colData(ipsc_mn_als_datasets$vsd)) %>% gsub("dafinca.tardbp", "dafinca",.) %>% gsub("dafinca.c9orf72", "dafinca", .) %>% gsub("mn_d25_", "", .)
...
dataset.samples.df = ipsc_mn_als_datasets.metadata %>% select(dataset_sample, dataset) %>% mutate(dataset_sample = gsub("dafinca.tardbp", "dafinca", dataset_sample), dataset_sample = gsub("dafinca.c9orf72", "dafinca", dataset_sample), dataset_sample = gsub("mn_d25_", "",dataset_sample), dataset = as.factor(dataset)) 
...
```{r}
ipsc_mn_als_datasets$res %>% filter(padj < 0.05)# 111
...
ipsc_mn_als_datasets.prefilt$res %>% filter(padj < 0.05) # 111

ipsc_mn_als_datasets.prefilt.degs= ipsc_mn_als_datasets.prefilt$res %>% filter(padj < 0.05) %>% pull(gene_name) # 111
ipsc_mn_als_datasets.degs = ipsc_mn_als_datasets$res %>% filter(padj < 0.05) %>% pull(gene_name) # 111
ipsc_mn_als_datasets.degs[ipsc_mn_als_datasets.degs %in% ipsc_mn_als_datasets.prefilt.degs]
...
# how many DEGs are on sex chromosomes 
ipsc_mn_als_datasets$res %>% filter(padj < 0.05) %>% left_join(gtf) %>% filter(chr %in% c("X","Y"))
...
ipsc_mn_als_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_als_datasets.geneList) + 
  ...
  annotate("text", x = 30000, y = 0.3, label = paste0("NES = ", round(ipsc_mn_als_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_als_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
...
ggsave(genetic_subgroups_scatter_upset, filename = here(proj_path,"expression/deseq2/genetic_subgroups_scatter_upset.png"), height = 10, width = 10)
...
```{r}
genetic_subgroups_compared.merged = plot_grid(ipsc_mn_sporadic_familial_stat.corr, ipsc_mn_mutant_sporadic.volcano,
                                              # ipsc_mn_tdp43_positive_negative_stat.corr, ipsc_mn_tdp_pathology.volcano,
                                              nrow = 1, ncol = 2, labels = "auto")
# genetic_subgroups_compared.merged
ggsave(genetic_subgroups_compared.merged, filename = here(proj_path,"expression/deseq2/genetic_subgroups_compared.merged.png"), height = 6, width = 10)
...
pan_als.voila_modulizer <- read_voila_modulizer(here(proj_path,"splicing/majiq/modulize/pan_als"), skip_rows = 9)
...
# 144/631
# pan_als.voila_modulizer %>% filter(denovo == TRUE, modulizer == "cassette_exon", abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% count(modulizer) #25 cryptic exons
# pan_als.voila_modulizer %>% filter(denovo == TRUE, modulizer == "cassette_exon", abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% arrange(-abs(als_ctrl_het_median_dpsi))  # FLNB, SDHAP1, SEC31B
...
modulizer_simplfied.sig.summary = pan_als.voila_modulizer %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% count(modulizer_simplified) %>% mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% 
...
# modulizer_simplfied.summary = pan_als.voila_modulizer %>% count(modulizer_simplified) %>% mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% mutate(pct = n/sum(n) *100, pct.label = case_when(pct > 5 ~ paste(as.character(n),"\n", as.character(round(pct,1)),"%"), TRUE ~ ""), ypos = cumsum(pct) - 0.5*pct) #%>% print
# modulizer_simplfied.summary %>% print
# pan_als.voila_modulizer.pie = ggplot(modulizer_simplfied.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + theme_void() + theme(legend.title=element_blank()) + geom_text(aes(y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1")
# pan_als.voila_modulizer.pie

# # Violin per splicing type
# pan_als.voila_modulizer %>% glimpse
# pan_als.voila_modulizer %>% my_summarise(tnom)
# pan_als.voila_modulizer.violin = violin_plot(filter(pan_als.voila_modulizer, modulizer_simplified != "other"), color_by = "modulizer_simplified", continuous = "als_ctrl_het_median_dpsi", cols = c("dodgerblue2","firebrick2","forestgreen","gold2","dodgerblue2","firebrick2","forestgreen","gold2","forestgreen","gold2"), ylims = c(-0.5,0.5), plot_stats = FALSE, stats_test = "wilcoxon_test", one_sample = TRUE, stat_ypos = 0.5, tip_length = 0.01, xlabel = "", arrow_labels = TRUE, numerator = "ALS", denomenator = "CTRL", dpi = 30)
# pan_als.voila_modulizer.violin

# # Cryptic Exons == denovo cassette_exon
# pan_als.voila_modulizer %>% filter(denovo == TRUE) %>% count(modulizer)
# pan_als.voila_modulizer %>% count(modulizer)
# pan_als.voila_modulizer %>% filter(denovo == TRUE, modulizer == "cassette_exon", abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% count(modulizer)
# pan_als.voila_modulizer %>% filter(denovo == TRUE, modulizer == "cassette_exon", abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% arrange(-abs(als_ctrl_het_median_dpsi)) %>% distinct(gene_name) %>% pull(gene_name) # "FLNB"          "SDHAP1"        "SEC31B"        "LIMS1"         "TPTEP2-CSNK1E" "ZNF721"        "PHKB"          "BCLAF3"        "SNX2"          "UGGT2"         "C1QTNF3-AMACR" "AC007566.1"    "CXXC4"         "FAM53B"        "ITGA6"         "R3HDM2"        "DELE1"         "NPIPA1"        "BABAM2"        "RALGAPA2"     
# pan_als.voila_modulizer %>% filter(denovo == TRUE, modulizer == "cassette_exon", abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_wilcoxon < 0.05) %>% distinct(gene_name) %>% pull(gene_name)
# pan_als.voila_modulizer %>% filter(denovo == TRUE, modulizer == "cassette_exon", gene_name %in% ALS_genes, abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_wilcoxon < 0.05) %>% distinct(gene_name) %>% pull(gene_name) # ATXN2
# 
# pan_als.voila_modulizer %>% filter(gene_name == "SFPQ") %>% arrange(-abs(als_ctrl_het_median_dpsi))
# pan_als.voila_modulizer %>% filter(gene_name %in% p53.signalling.pathway, abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% arrange(-abs(als_ctrl_het_median_dpsi))
# 
# # Intron retention 
# pan_als.voila_modulizer.ir = pan_als.voila_modulizer %>% filter(modulizer_simplified == "intron_retention")
# pan_als.voila_modulizer.ir %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% arrange(-abs(als_ctrl_het_median_dpsi)) # 417
# pan_als.voila_modulizer.ir %>% filter(gene_name %in% ALS_genes, abs(als_ctrl_het_median_dpsi) > 0.1, tnom < 0.05) %>% arrange(-abs(als_ctrl_het_median_dpsi))


# Voila View deltaPSI violin plot examples of top events:
als_ctrl.voila %>% filter(changing_dpsi0.1 == TRUE, gene_name %in% ALS_genes)
als_ctrl.voila %>% filter(changing_dpsi0.2 == TRUE, gene_name == "SRSF1") %>% glimpse # lsv_junction_id   ENSG00000136450:s:58005801-58005973:j:58005601-58005800
als_ctrl.voila %>% filter(changing_dpsi0.1 == TRUE, gene_name == "INPP5F") %>% glimpse # lsv_junction_id   ENSG00000198825:t:119820846-119820917:j:119811955-119820846
...
SRSF1_majiq_violins.png = magick::image_read(here(proj_path,"splicing/majiq/figures/SRSF1_majiq_violin.png"))
SRSF1_majiq_violins.gg = ggdraw() + draw_image(SRSF1_majiq_violins.png)
# SRSF1_majiq_violins.gg
majiq_violins.gg = SRSF1_majiq_violins.gg + CFAP410_majiq_violins.gg  + C9orf72_majiq_violins.gg + HDAC1_majiq_violins.gg
# majiq_violins.gg
...
# Violin
ipsc_mn_als_datasets.irfinder.filt = ipsc_mn_als_datasets$irfinder %>% filter(log2FoldChange != 0) %>% mutate(condition = "Intron retention\nALS vs CTRL")
ipsc_mn_als_datasets.ir.violin <- violin_plot(ipsc_mn_als_datasets.irfinder.filt, color_by = "condition", cols = "firebrick2", one_sample = TRUE, stats_test = "t_test", arrow_label_x = 0.87, numerator = "ALS", denomenator = "CTRL", ylims = c(-1.5,1.5), dpi = 300)
...
## Volcano
ipsc_mn_als_datasets.ir.als.labels <- ipsc_mn_als_datasets$irfinder %>% filter(abs(stat) > 5, gene_name %in% c(ALS_genes)) %>% arrange(-abs(stat)) %>% distinct(gene_name, .keep_all = TRUE) %>% top_n(8, wt = abs(stat))  %>% pull(gene_name)
ipsc_mn_als_datasets.ir.p53.labels <- ipsc_mn_als_datasets$irfinder %>% filter(abs(stat) > 5, log2FoldChange > 0.5, gene_name %in% c(p53.signalling.pathway)) %>% arrange(-abs(stat)) %>% distinct(gene_name, .keep_all = TRUE) %>% top_n(8, wt = abs(stat)) %>% pull(gene_name)
ipsc_mn_als_datasets.ir.synapse.labels <- ipsc_mn_als_datasets$irfinder %>% filter(padj < 0.05, gene_name %in% c(gprofiler_database$synapse)) %>% arrange(-abs(stat)) %>% distinct(gene_name, .keep_all = TRUE) %>% filter(stat < 0) %>% top_n(8, wt = abs(stat)) %>% pull(gene_name)
...
ipsc_mn_als_datasets.ir.volcano <- volcano_plot(ipsc_mn_als_datasets$irfinder, numerator = "ALS", denomenator = "CTRL",
               title = "Intron retention: pan ALS", subtitle = "101 Controls vs 318 ALS", ymax = 17, xmax = 2, dpi = 300,
...
## GSEA: DNA damage response, p53 signalling pathway
ipsc_mn_als_datasets.intronList <- ipsc_mn_als_datasets$irfinder %>% drop_na(stat) %>% pull(stat)
names(ipsc_mn_als_datasets.intronList) <- ipsc_mn_als_datasets$irfinder %>% drop_na(stat) %>% pull(gene_name) %>% as.character()
ipsc_mn_als_datasets.intronList = sort(ipsc_mn_als_datasets.intronList, decreasing = TRUE)

# DNA damage response
dna.damage_genes.list <- list("dna.damage" = ipsc_mn_als_datasets$res$gene_name[ipsc_mn_als_datasets$res$gene_name %in% gprofiler_database$`DNA Damage Response`])
ipsc_mn_als_datasets.dna.damage_fgseaRes <- fgsea(pathways = dna.damage_genes.list, stats = ipsc_mn_als_datasets.intronList)
ipsc_mn_als_datasets.dna.damage_fgseaRes
#       pathway       pval       padj   log2err        ES      NES size                               leadingEdge
# 1: dna.damage 0.02747435 0.02747435 0.3524879 0.4434927 1.351135   67 TNFRSF10B,CDKN1A,MDM2,SESN1,RRM2B,BAX,...

ipsc_mn_als_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_als_datasets.intronList) + 
  theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(DNA~damage~response)) + xlab("") +
  annotate("text", x = 30000, y = 0.3, label = paste0("NES = ", round(ipsc_mn_als_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_als_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
ipsc_mn_als_datasets.dna.damage.gseaplot
...
p53.signalling.pathway = unique(c(gprofiler_database$`TP53 Network`, go.pathways.msigdb$GO_P53_BINDING, go.pathways.msigdb$GO_POSITIVE_REGULATION_OF_SIGNAL_TRANSDUCTION_BY_P53_CLASS_MEDIATOR, go.pathways.msigdb$GO_POSITIVE_REGULATION_OF_DNA_DAMAGE_RESPONSE_SIGNAL_TRANSDUCTION_BY_P53_CLASS_MEDIATOR, go.pathways.msigdb$GO_POSITIVE_REGULATION_OF_INTRINSIC_APOPTOTIC_SIGNALING_PATHWAY_BY_P53_CLASS_MEDIATOR))
p53.signalling_genes.list <- list("p53.signalling" = ipsc_mn_als_datasets$res$gene_name[ipsc_mn_als_datasets$res$gene_name %in% p53.signalling.pathway])
ipsc_mn_als_datasets.p53.signalling_fgseaRes <- fgsea(pathways = p53.signalling_genes.list, stats = ipsc_mn_als_datasets.intronList)
ipsc_mn_als_datasets.p53.signalling_fgseaRes
#           pathway        pval        padj   log2err       ES      NES size                          leadingEdge
# 1: p53.signalling 0.005941836 0.005941836 0.4070179 0.455499 1.479662   95 CDKN1A,MDM2,TRIAP1,BAX,TAF3,BBC3,...
ipsc_mn_als_datasets.p53.signalling.gseaplot = plotEnrichment(p53.signalling_genes.list[["p53.signalling"]], ipsc_mn_als_datasets.intronList) + 
  theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(p53~signalling)) + xlab("") +
  annotate("text", x = 35000, y = 0.3, label = paste0("NES = ", round(ipsc_mn_als_datasets.p53.signalling_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_als_datasets.p53.signalling_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
ipsc_mn_als_datasets.p53.signalling.gseaplot
...
pan_als_ipsn_splicing_fig_merged = ggarrange(
  majiq_schematic.gg,
  ggarrange(majiq.als_vs_ctrl.volcano, majiq_violins.gg, ncol = 2, labels = c("b","c"), widths = c(1,0.8)),
  ggarrange(als_ctrl.voila.gost_filt.terms.curated, pan_als.voila_modulizer.sig.pie, ipsc_mn_als_datasets.ir.violin, ncol = 3, labels = c("d","e","f"), widths = c(0.5,1,0.5)),
  ggarrange(ipsc_mn_als_datasets.ir.volcano, ipsc_mn_als_datasets.ir.go_curated, ncol = 2, labels = c("g","h"), widths = c(1,0.8)),
  nrow = 4, heights = c(0.6,1.2,0.9,1.2), labels = c("a","","",""))
# pan_als_ipsn_splicing_fig_merged
ggsave(pan_als_ipsn_splicing_fig_merged, filename = here(proj_path,"expression/deseq2/pan_als_ipsn_splicing_fig_merged.png"), height = 11, width = 8.5)
...
# Fig S13 IR characteristics

```{r}
ipsc_mn_als_datasets.irfinder.filt = ipsc_mn_als_datasets$irfinder %>% filter(log2FoldChange != 0) %>% mutate(condition = "ALS vs CTRL", log10.intron_length = log10(intron_length))
gr_introns <- as.GenomicRange(ipsc_mn_als_datasets.irfinder.filt$Intron.GeneName.GeneID.Coords)
seqlevelsStyle(gr_introns) <- "UCSC"
ipsc_mn_als_datasets.irfinder.filt$phast_cons <- gscores(phastCons100way.UCSC.hg38, gr_introns) %>% as_tibble %>% pull(default)
iclip_score.HepG2_K562_Hela.normalised <-readRDS(here(camp_path,"genomes/rbp-databases/clip-bedgraphs/iclip_score.HepG2_K562_Hela.intron.normalised.rds")) %>% select(Intron.GeneName.GeneID.Coords, mean_iclip_score, sum_iclip_score)
ipsc_mn_als_datasets.irfinder.filt = ipsc_mn_als_datasets.irfinder.filt %>% left_join(iclip_score.HepG2_K562_Hela.normalised) %>%
  mutate(log10.phast_cons = log10(phast_cons), log10.mean_iclip_score = log10(mean_iclip_score), gc_content = gc_content[,1])

# GC content -------------------------------
ipsc_mn_als_datasets.ir_gc_violin = violin_plot(filter(ipsc_mn_als_datasets.irfinder.filt, padj < 0.05), color_by = "IRdirection", continuous = "gc_content", null_value = 0.5, ylims = c(0.2,0.8), ylabel = "GC content", arrow_labels = FALSE, stats_test = "wilcox_test", dpi = 300)
# ipsc_mn_als_datasets.ir_gc_violin
# .y.   group1 group2    n1     n2 statistic    df            p p.signif
#   <chr> <chr>  <chr>  <int>  <int>     <dbl> <dbl>        <dbl> <chr>   
# 1 lfc   down   up      2902 112378      5.39 3052. 0.0000000744 ****    
# # A tibble: 1 × 7
#   .y.   group1 group2 effsize    n1     n2 magnitude 
# * <chr> <chr>  <chr>    <dbl> <int>  <int> <ord>     
# 1 lfc   down   up       0.102  2902 112378 negligible

## intron_length -------------------------------
ipsc_mn_als_datasets.ir_length_violin = violin_plot(filter(ipsc_mn_als_datasets.irfinder.filt, padj < 0.05), color_by = "IRdirection", continuous = "log10.intron_length", null_value = 0.5, ylims = c(1.8,6.1), ylabel = "log10 Intron Length", stats_test = "wilcox_test", arrow_labels = FALSE, dpi = 300)
# ipsc_mn_als_datasets.ir_length_violin
# [1] "Using two sample t_test"
# # A tibble: 1 × 9
#   .y.   group1 group2    n1     n2 statistic    df        p p.signif
#   <chr> <chr>  <chr>  <int>  <int>     <dbl> <dbl>    <dbl> <chr>   
# 1 lfc   down   up      2902 112378      17.9 3001. 1.91e-68 ****    
# # A tibble: 1 × 7
#   .y.   group1 group2 effsize    n1     n2 magnitude
# * <chr> <chr>  <chr>    <dbl> <int>  <int> <ord>    
# 1 lfc   down   up       0.411  2902 112378 small    

## PhastCons -------------------------------
ipsc_mn_als_datasets.ir_phast_violin = violin_plot(filter(ipsc_mn_als_datasets.irfinder.filt, padj < 0.05), color_by = "IRdirection", continuous = "log10.phast_cons", null_value = 1, ylims = c(-2.6,0), ylabel = "log10 Phast Conservation score", arrow_labels = FALSE, stats_test = "wilcox_test", dpi = 300)
# ipsc_mn_als_datasets.ir_phast_violin
# [1] "Using two sample t_test"
# # A tibble: 1 × 9
#   .y.   group1 group2    n1     n2 statistic    df         p p.signif
#   <chr> <chr>  <chr>  <int>  <int>     <dbl> <dbl>     <dbl> <chr>   
# 1 lfc   down   up      2646 108688     -4.58 2766. 0.0000048 ****    
# # A tibble: 1 × 7
#   .y.   group1 group2 effsize    n1     n2 magnitude 
# * <chr> <chr>  <chr>    <dbl> <int>  <int> <ord>     
# 1 lfc   down   up     -0.0933  2902 112378 negligible
# CLIP -------------------------------
ipsc_mn_als_datasets.ir_clip_violin = violin_plot(filter(ipsc_mn_als_datasets.irfinder.filt, padj < 0.05), color_by = "padj0.05_IRdirection", continuous = "log10.mean_iclip_score", null_value = 1, ylims = c(-6,-0.4), stat_ypos = -0.8, ylabel = "log10 RBP binding score", arrow_labels = FALSE, dpi = 300)
# ipsc_mn_als_datasets.ir_clip_violin
# [1] "Using two sample t_test"
# # A tibble: 1 × 9
#   .y.   group1 group2    n1     n2 statistic    df     p p.signif
#   <chr> <chr>  <chr>  <int>  <int>     <dbl> <dbl> <dbl> <chr>   
# 1 lfc   down   up      2793 111596     -58.7 2854.     0 ****    
# # A tibble: 1 × 7
#   .y.   group1 group2 effsize    n1     n2 magnitude
# * <chr> <chr>  <chr>    <dbl> <int>  <int> <ord>    
# 1 lfc   down   up       -1.66  2902 112378 large    

# MaxEntScan 5' scores -------------------------------
ss_motif <- readRDS(here(camp_path,"projects/ac-mn-comparison/splicing/IRFinder/MaxEntScan/irfinder.introns.maxentscant.RDS"))
ipsc_mn_als_datasets.irfinder.filt <- ipsc_mn_als_datasets.irfinder.filt %>% left_join(select(ss_motif, Chr, Start, End, Direction, MaxEntScore_5ss, MaxEntScore_3ss), by = c("Chr", "Start", "End", "Direction")) %>%
...
ipsc_mn_als_datasets.ir_5ss_violin = violin_plot(filter(ipsc_mn_als_datasets.irfinder.filt, padj < 0.05), color_by = "padj0.05_IRdirection", continuous = "MaxEntScore_5ss", null_value = -1, ylims = c(0,13), ylabel = "5' splice site strength", arrow_labels = FALSE, stats_test = "wilcox_test", dpi = 300)
# ipsc_mn_als_datasets.ir_5ss_violin
# [1] "Using two sample t_test"
# # A tibble: 1 × 9
#   .y.   group1 group2    n1    n2 statistic    df       p p.signif
#   <chr> <chr>  <chr>  <int> <int>     <dbl> <dbl>   <dbl> <chr>   
# 1 lfc   down   up      1018 71182      2.67 1052. 0.00759 **      
# # A tibble: 1 × 7
#   .y.   group1 group2 effsize    n1     n2 magnitude 
# * <chr> <chr>  <chr>    <dbl> <int>  <int> <ord>     
# 1 lfc   down   up      0.0772  2974 118119 negligible

# MaxEntScan 3' scores -------------------------------
ipsc_mn_als_datasets.ir_3ss_violin = violin_plot(filter(ipsc_mn_als_datasets.irfinder.filt, padj < 0.05), color_by = "padj0.05_IRdirection", continuous = "MaxEntScore_3ss", null_value = -1, ylims = c(0,15), ylabel = "3' splice site strength", arrow_labels = FALSE, stats_test = "wilcox_test", dpi = 300)
# ipsc_mn_als_datasets.ir_3ss_violin
# [1] "Using two sample t_test"
# # A tibble: 1 × 9
#   .y.   group1 group2    n1    n2 statistic    df        p p.signif
#   <chr> <chr>  <chr>  <int> <int>     <dbl> <dbl>    <dbl> <chr>   
# 1 lfc   down   up      1018 71182      9.97 1056. 1.98e-22 ****    
# # A tibble: 1 × 7
#   .y.   group1 group2 effsize    n1     n2 magnitude
# * <chr> <chr>  <chr>    <dbl> <int>  <int> <ord>    
# 1 lfc   down   up       0.275  2974 118119 small    


# Merge
ipsc_mn_als_datasets.ir_characteristics.merged = plot_grid(
  ipsc_mn_als_datasets.ir_gc_violin, ipsc_mn_als_datasets.ir_length_violin,
  ipsc_mn_als_datasets.ir_phast_violin, ipsc_mn_als_datasets.ir_clip_violin,
  ipsc_mn_als_datasets.ir_5ss_violin, ipsc_mn_als_datasets.ir_3ss_violin,
  nrow = 3, ncol = 2, labels = "auto")
# ipsc_mn_als_datasets.ir_characteristics.merged
ggsave(ipsc_mn_als_datasets.ir_characteristics.merged, filename = here(proj_path,"splicing/irfinder/ipsc_mn_als_datasets.ir_characteristics.merged.png"), height = 11, width = 8.25)

```



# Fig 6 Splicing compare mutations
...
c9orf72.voila_modulizer <- read_voila_modulizer(here(proj_path,"splicing/majiq/modulize/c9orf72"), skip_rows = 9)
fus.voila_modulizer <- read_voila_modulizer(here(proj_path,"splicing/majiq/modulize/fus"), skip_rows = 9)
tardbp.voila_modulizer <- read_voila_modulizer(here(proj_path,"splicing/majiq/modulize/tardbp"), skip_rows = 9)
sod1.voila_modulizer <- read_voila_modulizer(here(proj_path,"splicing/majiq/modulize/sod1"), skip_rows = 9)


sporadic.modulizer.sig.summary = sporadic_ctrl.voila_modulizer %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_tnom < 0.05) %>% count(modulizer_simplified) %>% 
  mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% 
  mutate(pct = n/sum(n) *100, pct.label = case_when(pct > 6.5 ~ paste(as.character(n),"\n", as.character(round(pct,1)),"%"), TRUE ~ ""), ypos = cumsum(pct) - 0.5*pct, group = "Sporadic")
c9orf72.modulizer.sig.summary = c9orf72.voila_modulizer %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_tnom < 0.05) %>% count(modulizer_simplified) %>% 
  mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% 
  mutate(pct = n/sum(n) *100, pct.label = case_when(pct > 6.5 ~ paste(as.character(n),"\n", as.character(round(pct,1)),"%"), TRUE ~ ""), ypos = cumsum(pct) - 0.5*pct, group = "C9orf72")
tardbp.modulizer.sig.summary = tardbp.voila_modulizer %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_tnom < 0.05) %>% count(modulizer_simplified) %>% 
  mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% 
  mutate(pct = n/sum(n) *100, pct.label = case_when(pct > 6.5 ~ paste(as.character(n),"\n", as.character(round(pct,1)),"%"), TRUE ~ ""), ypos = cumsum(pct) - 0.5*pct, group = "TARDBP")
fus.modulizer.sig.summary = fus.voila_modulizer %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_tnom < 0.05) %>% count(modulizer_simplified) %>% 
  mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% 
  mutate(pct = n/sum(n) *100, pct.label = case_when(pct > 6.5 ~ paste(as.character(n),"\n", as.character(round(pct,1)),"%"), TRUE ~ ""), ypos = cumsum(pct) - 0.5*pct, group = "FUS")
sod1.modulizer.sig.summary = sod1.voila_modulizer %>% filter(abs(als_ctrl_het_median_dpsi) > 0.1, als_ctrl_het_tnom < 0.05) %>% count(modulizer_simplified) %>% 
  mutate(modulizer_simplified = fct_reorder(modulizer_simplified, n, .desc = TRUE)) %>% arrange(desc(modulizer_simplified)) %>% 
  mutate(pct = n/sum(n) *100, pct.label = case_when(pct > 6.5 ~ paste(as.character(n),"\n", as.character(round(pct,1)),"%"), TRUE ~ ""), ypos = cumsum(pct) - 0.5*pct, group = "SOD1")

genetic_subgroups.modulizer.sig.summary = bind_rows(c9orf72.modulizer.sig.summary, tardbp.modulizer.sig.summary, fus.modulizer.sig.summary, sod1.modulizer.sig.summary) %>% # sporadic.modulizer.sig.summary
  mutate(group = factor(group, levels = c("Sporadic", "TARDBP", "FUS", "C9orf72", "SOD1"))) %>%
  filter(modulizer_simplified != "orphan_junction")
genetic_subgroups.modulizer.sig.pie = ggplot(genetic_subgroups.modulizer.sig.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + 
  facet_wrap(~ group, nrow = 1) +
  geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + 
  geom_text(aes(x = 1.2, y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1") + 
  theme_oz() + theme_void() + theme(legend.title=element_blank(), legend.position = "right", legend.text=element_text(size=7), legend.spacing.y = unit(1, 'mm'), legend.spacing.x = unit(1, 'mm'), legend.key.height = unit(0.4, 'cm'), legend.key.width = unit(0.3, 'cm')) + guides(fill=guide_legend(ncol=1))
genetic_subgroups.modulizer.sig.pie

# sporadic.voila.sig.pie = ggplot(sporadic.modulizer.sig.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + 
#   geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + 
#   geom_text(aes(x = 1.2, y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1") + 
#   theme_oz() + theme_void() + theme(legend.title=element_blank(), legend.position = "right", legend.text=element_text(size=7), legend.spacing.y = unit(1, 'mm'), legend.spacing.x = unit(1, 'mm'), legend.key.height = unit(0.4, 'cm'), legend.key.width = unit(0.3, 'cm')) + guides(fill=guide_legend(ncol=1))
# # sporadic.voila.sig.pie
# 
# c9orf72.voila.sig.pie = ggplot(c9orf72.modulizer.sig.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + 
#   geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + 
#   geom_text(aes(x = 1.2, y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1") + 
#   theme_oz() + theme_void() + theme(legend.title=element_blank(), legend.position = "right", legend.text=element_text(size=7), legend.spacing.y = unit(1, 'mm'), legend.spacing.x = unit(1, 'mm'), legend.key.height = unit(0.4, 'cm'), legend.key.width = unit(0.3, 'cm')) + guides(fill=guide_legend(ncol=1))
# c9orf72.voila.sig.pie
# 
# tardbp.voila.sig.pie = ggplot(tardbp.modulizer.sig.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + 
#   geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + 
#   geom_text(aes(x = 1.2, y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1") + 
#   theme_oz() + theme_void() + theme(legend.title=element_blank(), legend.position = "right", legend.text=element_text(size=7), legend.spacing.y = unit(1, 'mm'), legend.spacing.x = unit(1, 'mm'), legend.key.height = unit(0.4, 'cm'), legend.key.width = unit(0.3, 'cm')) + guides(fill=guide_legend(ncol=1))
# # tardbp.voila.sig.pie
# 
# fus.voila.sig.pie = ggplot(fus.modulizer.sig.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + 
#   geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + 
#   geom_text(aes(x = 1.2, y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1") + 
#   theme_oz() + theme_void() + theme(legend.title=element_blank(), legend.position = "right", legend.text=element_text(size=7), legend.spacing.y = unit(1, 'mm'), legend.spacing.x = unit(1, 'mm'), legend.key.height = unit(0.4, 'cm'), legend.key.width = unit(0.3, 'cm')) + guides(fill=guide_legend(ncol=1))
# # fus.voila.sig.pie
# 
# sod1.voila.sig.pie = ggplot(sod1.modulizer.sig.summary, aes(x = "", y = pct, fill = fct_reorder(modulizer_simplified, n, .desc = TRUE))) + 
#   geom_bar(width = 1, stat = "identity", color="white") + coord_polar("y", start=0) + 
#   geom_text(aes(x = 1.2, y = ypos, label = pct.label), color = "black", size=2.5) + scale_fill_brewer(palette="Set1") + 
#   theme_oz() + theme_void() + theme(legend.title=element_blank(), legend.position = "right", legend.text=element_text(size=7), legend.spacing.y = unit(1, 'mm'), legend.spacing.x = unit(1, 'mm'), legend.key.height = unit(0.4, 'cm'), legend.key.width = unit(0.3, 'cm')) + guides(fill=guide_legend(ncol=1))
# # sod1.voila.sig.pie
# 
# genetics_subgroups.modulizer.piecharts = plot_grid(tardbp.voila.sig.pie, fus.voila.sig.pie, c9orf72.voila.sig.pie, sod1.voila.sig.pie, nrow = 2, ncol = 3, labels = "auto") # sporadic.voila.sig.pie
# genetics_subgroups.modulizer.piecharts
...
compare_mutations_ipsn_splicing_fig_merged = ggarrange(
  ...
  genetic_subgroups.modulizer.sig.pie, #ipsc_mn_mutations_datasets.irfinder.violin,
...
# Fig S13 MAJIQ mutations scatters & upset
...
# Fig S14 IRFinder compare mutations
...
ipsc_mn_mutants_bind_datasets.irfinder.upset_intron_level = ipsc_mn_mutations_irfinder_bind %>% filter(padj < 0.05) %>% select(mutation, Intron.GeneName.GeneID.Coords) %>% group_by(Intron.GeneName.GeneID.Coords) %>% summarise(mutations = list(mutation)) %>%
  ggplot(aes(x = mutations)) + geom_bar() +
    geom_text(stat='count', aes(label=after_stat(count)), vjust=-1, hjust = 0.5, size = 3) + #, angle = 30) +
    scale_x_upset(order_by = "degree")  + # n_intersections = 20
    scale_y_continuous(breaks = NULL, lim = c(0, 130000), name = "") +
    theme_classic() +  theme(legend.title = element_blank(), legend.position = "none", axis.text=element_text(size=8), axis.title=element_text(size=10)) + labs(x = "", y = "") +
    theme_combmatrix(combmatrix.label.make_space = TRUE)
...
```{r}
# GO
ipsc_mn_sporadic_datasets.gost <- ipsc_mn_sporadic_datasets$irfinder %>% filter(padj < 0.05) %>% arrange(-abs(stat)) %>% distinct(gene_id, .keep_all = TRUE) %>% group_split(log2FoldChange > 0) %>% map("gene_id") %>% gost()
ipsc_mn_sporadic_datasets.gost_filt <- ipsc_mn_sporadic_datasets.gost$result %>% filter(str_detect(source, "KEGG|GO:BP|GO:MF|GO:CC")) %>% arrange( -log10(p_value) ) %>%
  mutate(query = case_when(query == "query_1" ~ "sporadic\nIR down", query == "query_2" ~ "sporadic\nIR up"), query = factor(query, levels = c("sporadic\nIR down", "sporadic\nIR up")))
table(ipsc_mn_sporadic_datasets.gost_filt$query)
# sporadic\nIR down   sporadic\nIR up 
#                2              940
ipsc_mn_sporadic_datasets.gost_ir.down <- ipsc_mn_sporadic_datasets.gost_filt %>% filter(query == "sporadic\nIR down")
ipsc_mn_sporadic_datasets.gost_ir.up <- ipsc_mn_sporadic_datasets.gost_filt %>% filter(query == "sporadic\nIR up")
paste('"',unique(ipsc_mn_sporadic_datasets.gost_ir.down$term_name),'",', sep = "", collapse = '\n') %>% cat()
down_list = c(
"node of Ranvier",
"axon initial segment")
ipsc_mn_sporadic_datasets.gost_filt_ir_down <- ipsc_mn_sporadic_datasets.gost_filt %>% filter(query == "sporadic\nIR down", term_name %in% down_list)
paste('"',unique(ipsc_mn_sporadic_datasets.gost_ir.up$term_name),'",', sep = "", collapse = '\n') %>% cat()
up_list = c(
"mitochondrion",
"mRNA transport",
"DNA repair",
...
"DNA metabolic process",
"proteasomal protein catabolic process")
ipsc_mn_sporadic_datasets.gost_filt_ir_up <- ipsc_mn_sporadic_datasets.gost_filt %>% filter(query == "sporadic\nIR up")  %>% filter(term_name %in% up_list)
ipsc_mn_sporadic_datasets.gost_filt_ir = bind_rows(ipsc_mn_sporadic_datasets.gost_filt_ir_up, ipsc_mn_sporadic_datasets.gost_filt_ir_down) %>% mutate(query = fct_relevel(query,"sporadic\nIR up"))
ipsc_mn_sporadic_datasets.gost_filt_ir$term_name = gsub("synaptic transmission, glutamatergic", "glutamatergic synapse", ipsc_mn_sporadic_datasets.gost_filt_ir$term_name) %>% gsub("cation channel activity", "channel", .) %>% gsub("regulation of ", "", .) %>% gsub("protein catabolic process", "process", .) %>% gsub("cellular ", "", .) %>% gsub("response to DNA damage stimulus", "DNA damage response",.)
ipsc_mn_sporadic_datasets.ir.go_curated <- curated_profiles(ipsc_mn_sporadic_datasets.gost_filt_ir, cols = c("firebrick2", "dodgerblue2")) + theme(strip.text.y.right = element_text(angle = 0)) + coord_cartesian(xlim = c(0,6))
ipsc_mn_sporadic_datasets.ir.go_curated


ipsc_mn_fus_datasets.gost <- ipsc_mn_fus_datasets$irfinder %>% filter(padj < 0.05) %>% arrange(-abs(stat)) %>% distinct(gene_id, .keep_all = TRUE) %>% group_split(log2FoldChange > 0) %>% map("gene_id") %>% gost()
ipsc_mn_fus_datasets.gost_filt <- ipsc_mn_fus_datasets.gost$result %>% filter(str_detect(source, "KEGG|GO:BP|GO:MF|GO:CC")) %>% arrange( -log10(p_value) ) %>%
  mutate(query = case_when(query == "query_1" ~ "fus\nIR down", query == "query_2" ~ "fus\nIR up"), query = factor(query, levels = c("fus\nIR down", "fus\nIR up")))
table(ipsc_mn_fus_datasets.gost_filt$query)
# fus\nIR down   fus\nIR up 
#         2006           62 
ipsc_mn_fus_datasets.gost_ir.down <- ipsc_mn_fus_datasets.gost_filt %>% filter(query == "fus\nIR down")
ipsc_mn_fus_datasets.gost_ir.up <- ipsc_mn_fus_datasets.gost_filt %>% filter(query == "fus\nIR up")
paste('"',unique(ipsc_mn_fus_datasets.gost_ir.down$term_name),'",', sep = "", collapse = '\n') %>% cat()
down_list = c(
"mitochondrion",
"mRNA transport",
"DNA repair",
...
"DNA metabolic process",
"proteasomal protein catabolic process")
ipsc_mn_fus_datasets.gost_filt_ir_down <- ipsc_mn_fus_datasets.gost_filt %>% filter(query == "fus\nIR down", term_name %in% down_list)
paste('"',unique(ipsc_mn_fus_datasets.gost_ir.up$term_name),'",', sep = "", collapse = '\n') %>% cat()
up_list = c(
"GABA-ergic synapse",
"neuron projection",
"ion channel complex",
...
"Ribosome",
"synapse")
ipsc_mn_fus_datasets.gost_filt_ir_up <- ipsc_mn_fus_datasets.gost_filt %>% filter(query == "fus\nIR up")  %>% filter(term_name %in% up_list)
ipsc_mn_fus_datasets.gost_filt_ir = bind_rows(ipsc_mn_fus_datasets.gost_filt_ir_up, ipsc_mn_fus_datasets.gost_filt_ir_down) %>% mutate(query = fct_relevel(query,"fus\nIR up"))
ipsc_mn_fus_datasets.gost_filt_ir$term_name = gsub("synaptic transmission, glutamatergic", "glutamatergic synapse", ipsc_mn_fus_datasets.gost_filt_ir$term_name) %>% gsub("cation channel activity", "channel", .) %>% gsub("regulation of ", "", .) %>% gsub("protein catabolic process", "process", .) %>% gsub("cellular ", "", .) %>% gsub("response to DNA damage stimulus", "DNA damage response",.)
ipsc_mn_fus_datasets.ir.go_curated <- curated_profiles(ipsc_mn_fus_datasets.gost_filt_ir, cols = c("firebrick2", "dodgerblue2")) + theme(strip.text.y.right = element_text(angle = 0)) + coord_cartesian(xlim = c(0,6))
ipsc_mn_fus_datasets.ir.go_curated

ipsc_mn_tardbp_datasets.gost <- ipsc_mn_tardbp_datasets$irfinder %>% filter(padj < 0.05) %>% arrange(-abs(stat)) %>% distinct(gene_id, .keep_all = TRUE) %>% group_split(log2FoldChange > 0) %>% map("gene_id") %>% gost()
ipsc_mn_tardbp_datasets.gost_filt <- ipsc_mn_tardbp_datasets.gost$result %>% filter(str_detect(source, "KEGG|GO:BP|GO:MF|GO:CC")) %>% arrange( -log10(p_value) ) %>%
  mutate(query = case_when(query == "query_1" ~ "tardbp\nIR down", query == "query_2" ~ "tardbp\nIR up"), query = factor(query, levels = c("tardbp\nIR down", "tardbp\nIR up")))
table(ipsc_mn_tardbp_datasets.gost_filt$query)
# tardbp\nIR down   tardbp\nIR up 
#            1709             324 
ipsc_mn_tardbp_datasets.gost_ir.down <- ipsc_mn_tardbp_datasets.gost_filt %>% filter(query == "tardbp\nIR down")
ipsc_mn_tardbp_datasets.gost_ir.up <- ipsc_mn_tardbp_datasets.gost_filt %>% filter(query == "tardbp\nIR up")
paste('"',unique(ipsc_mn_tardbp_datasets.gost_ir.down$term_name),'",', sep = "", collapse = '\n') %>% cat()
down_list = c(
"mitochondrion",
"mRNA transport",
"DNA repair",
...
"DNA metabolic process",
"proteasomal protein catabolic process")
ipsc_mn_tardbp_datasets.gost_filt_ir_down <- ipsc_mn_tardbp_datasets.gost_filt %>% filter(query == "tardbp\nIR down", term_name %in% down_list)
paste('"',unique(ipsc_mn_tardbp_datasets.gost_ir.up$term_name),'",', sep = "", collapse = '\n') %>% cat()
up_list = c(
"dendrite",
"glutamatergic synapse",
"neuron differentiation",
...
"neuron projection",
"synapse")
ipsc_mn_tardbp_datasets.gost_filt_ir_up <- ipsc_mn_tardbp_datasets.gost_filt %>% filter(query == "tardbp\nIR up")  %>% filter(term_name %in% up_list)
ipsc_mn_tardbp_datasets.gost_filt_ir = bind_rows(ipsc_mn_tardbp_datasets.gost_filt_ir_up, ipsc_mn_tardbp_datasets.gost_filt_ir_down) %>% mutate(query = fct_relevel(query,"tardbp\nIR up"))
ipsc_mn_tardbp_datasets.gost_filt_ir$term_name = gsub("synaptic transmission, glutamatergic", "glutamatergic synapse", ipsc_mn_tardbp_datasets.gost_filt_ir$term_name) %>% gsub("cation channel activity", "channel", .) %>% gsub("regulation of ", "", .) %>% gsub("protein catabolic process", "process", .) %>% gsub("cellular ", "", .) %>% gsub("response to DNA damage stimulus", "DNA damage response",.)
ipsc_mn_tardbp_datasets.ir.go_curated <- curated_profiles(ipsc_mn_tardbp_datasets.gost_filt_ir, cols = c("firebrick2", "dodgerblue2")) + theme(strip.text.y.right = element_text(angle = 0)) + coord_cartesian(xlim = c(0,6))
ipsc_mn_tardbp_datasets.ir.go_curated

# ipsc_mn_sod1_datasets.gost <- ipsc_mn_sod1_datasets$irfinder %>% filter(padj < 0.05) %>% arrange(-abs(stat)) %>% distinct(gene_id, .keep_all = TRUE) %>% group_split(log2FoldChange > 0) %>% map("gene_id") %>% gost()
# ipsc_mn_sod1_datasets.gost_filt <- ipsc_mn_sod1_datasets.gost$result %>% filter(str_detect(source, "KEGG|GO:BP|GO:MF|GO:CC")) %>% arrange( -log10(p_value) ) %>%
#   mutate(query = case_when(query == "query_1" ~ "sod1\nIR down", query == "query_2" ~ "sod1\nIR up"), query = factor(query, levels = c("sod1\nIR down", "sod1\nIR up")))
# table(ipsc_mn_sod1_datasets.gost_filt$query)
# # sod1\nIR down   sod1\nIR up 
# #            1             0 
# ipsc_mn_sod1_datasets.ir.go_curated <- curated_profiles(ipsc_mn_sod1_datasets.gost_filt, cols = c("firebrick2", "dodgerblue2")) + theme(strip.text.y.right = element_text(angle = 0)) + coord_cartesian(xlim = c(0,6))
# ipsc_mn_sod1_datasets.ir.go_curated
# 
# 
# ipsc_mn_c9orf72_datasets.gost <- ipsc_mn_c9orf72_datasets$irfinder %>% filter(padj < 0.05) %>% arrange(-abs(stat)) %>% distinct(gene_id, .keep_all = TRUE) %>% group_split(log2FoldChange > 0) %>% map("gene_id") %>% gost()
# ipsc_mn_c9orf72_datasets.gost_filt <- ipsc_mn_c9orf72_datasets.gost$result %>% filter(str_detect(source, "KEGG|GO:BP|GO:MF|GO:CC")) %>% arrange( -log10(p_value) ) %>%
#   mutate(query = case_when(query == "query_1" ~ "c9orf72\nIR down", query == "query_2" ~ "c9orf72\nIR up"), query = factor(query, levels = c("c9orf72\nIR down", "c9orf72\nIR up")))
# table(ipsc_mn_c9orf72_datasets.gost_filt$query)
# # c9orf72\nIR down   c9orf72\nIR up 
# #                2                0 
# ipsc_mn_c9orf72_datasets.gost_ir.down <- ipsc_mn_c9orf72_datasets.gost_filt %>% filter(query == "c9orf72\nIR down")
# ipsc_mn_c9orf72_datasets.ir.go_curated <- curated_profiles(ipsc_mn_c9orf72_datasets.gost_filt, cols = c("firebrick2", "dodgerblue2")) + theme(strip.text.y.right = element_text(angle = 0)) + coord_cartesian(xlim = c(0,6))
# ipsc_mn_c9orf72_datasets.ir.go_curated



# ## GSEA sporadic: DNA damage response, p53 signalling pathway
# ipsc_mn_sporadic_datasets.intronList <- ipsc_mn_sporadic_datasets$irfinder %>% drop_na(stat) %>% pull(stat)
# names(ipsc_mn_sporadic_datasets.intronList) <- ipsc_mn_sporadic_datasets$irfinder %>% drop_na(stat) %>% pull(gene_name) %>% as.character()
# ipsc_mn_sporadic_datasets.intronList = sort(ipsc_mn_sporadic_datasets.intronList, decreasing = TRUE)
# 
# # DNA damage response
# dna.damage_genes.list <- list("dna.damage" = ipsc_mn_sporadic_datasets$irfinder$gene_name[ipsc_mn_sporadic_datasets$irfinder$gene_name %in% gprofiler_database$`DNA Damage Response`])
# ipsc_mn_sporadic_datasets.dna.damage_fgseaRes <- fgsea(pathways = dna.damage_genes.list, stats = ipsc_mn_sporadic_datasets.intronList)
# ipsc_mn_sporadic_datasets.dna.damage_fgseaRes
# # pathway         pval         padj  log2err       ES      NES size                      leadingEdge
# # 1: dna.damage 2.065871e-08 2.065871e-08 0.733762 0.667868 1.679483   64 CDK5,ATR,ATM,PRKDC,MDM2,TLK1,...
# ipsc_mn_sporadic_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_sporadic_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(DNA~damage~response)) + xlab("") +
#   annotate("text", x = 180000, y = 0.5, label = paste0("NES = ", round(ipsc_mn_sporadic_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_sporadic_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_sporadic_datasets.dna.damage.gseaplot
# 
# # p53 signalling pathway
# p53.signalling_genes.list <- list("p53.signalling" = ipsc_mn_sporadic_datasets$irfinder$gene_name[ipsc_mn_sporadic_datasets$irfinder$gene_name %in% p53.signalling.pathway])
# ipsc_mn_sporadic_datasets.p53.signalling_fgseaRes <- fgsea(pathways = p53.signalling_genes.list, stats = ipsc_mn_sporadic_datasets.intronList)
# ipsc_mn_sporadic_datasets.p53.signalling_fgseaRes
# #  pathway         pval         padj   log2err        ES      NES size                          leadingEdge
# # 1: p53.signalling 9.017292e-14 9.017292e-14 0.9545416 0.6903256 1.772842   94 CDK5,ATR,HSPD1,PTTG1IP,EHMT1,ATM,...
# ipsc_mn_sporadic_datasets.p53.signalling.gseaplot = plotEnrichment(p53.signalling_genes.list[["p53.signalling"]], ipsc_mn_sporadic_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(p53~signalling)) + xlab("") +
#   annotate("text", x = 180000, y = 0.6, label = paste0("NES = ", round(ipsc_mn_sporadic_datasets.p53.signalling_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_sporadic_datasets.p53.signalling_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_sporadic_datasets.p53.signalling.gseaplot
# 
# 
# ## GSEA c9orf72: DNA damage response, p53 signalling pathway
# ipsc_mn_c9orf72_datasets.intronList <- ipsc_mn_c9orf72_datasets$irfinder %>% drop_na(stat) %>% pull(stat)
# names(ipsc_mn_c9orf72_datasets.intronList) <- ipsc_mn_c9orf72_datasets$irfinder %>% drop_na(stat) %>% pull(gene_name) %>% as.character()
# ipsc_mn_c9orf72_datasets.intronList = sort(ipsc_mn_c9orf72_datasets.intronList, decreasing = TRUE)
# 
# # DNA damage response
# dna.damage_genes.list <- list("dna.damage" = ipsc_mn_c9orf72_datasets$irfinder$gene_name[ipsc_mn_c9orf72_datasets$irfinder$gene_name %in% gprofiler_database$`DNA Damage Response`])
# ipsc_mn_c9orf72_datasets.dna.damage_fgseaRes <- fgsea(pathways = dna.damage_genes.list, stats = ipsc_mn_c9orf72_datasets.intronList)
# ipsc_mn_c9orf72_datasets.dna.damage_fgseaRes
# # pathway pval padj log2err        ES NES size                         leadingEdge
# # 1: dna.damage   NA   NA      NA 0.3529883  NA   64 RAD17,TLK1,ATR,RRM2B,TP53,CHEK1,...
# ipsc_mn_c9orf72_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_c9orf72_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(DNA~damage~response)) + xlab("") +
#   annotate("text", x = 180000, y = 0.5, label = paste0("NES = ", round(ipsc_mn_c9orf72_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_c9orf72_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_c9orf72_datasets.dna.damage.gseaplot
# 
# # p53 signalling pathway
# p53.signalling_genes.list <- list("p53.signalling" = ipsc_mn_c9orf72_datasets$irfinder$gene_name[ipsc_mn_c9orf72_datasets$irfinder$gene_name %in% p53.signalling.pathway])
# ipsc_mn_c9orf72_datasets.p53.signalling_fgseaRes <- fgsea(pathways = p53.signalling_genes.list, stats = ipsc_mn_c9orf72_datasets.intronList)
# ipsc_mn_c9orf72_datasets.p53.signalling_fgseaRes
# #  pathway pval padj log2err        ES NES size                             leadingEdge
# # 1: p53.signalling   NA   NA      NA 0.3832425  NA   94 ATR,CHD5,TP53,TP53BP2,MUC1,CDK5RAP3,...
# ipsc_mn_c9orf72_datasets.p53.signalling.gseaplot = plotEnrichment(p53.signalling_genes.list[["p53.signalling"]], ipsc_mn_c9orf72_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(p53~signalling)) + xlab("") +
#   annotate("text", x = 180000, y = 0.6, label = paste0("NES = ", round(ipsc_mn_c9orf72_datasets.p53.signalling_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_c9orf72_datasets.p53.signalling_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_c9orf72_datasets.p53.signalling.gseaplot
# 
# 
# ## GSEA sod1: DNA damage response, p53 signalling pathway
# ipsc_mn_sod1_datasets.intronList <- ipsc_mn_sod1_datasets$irfinder %>% drop_na(stat) %>% pull(stat)
# names(ipsc_mn_sod1_datasets.intronList) <- ipsc_mn_sod1_datasets$irfinder %>% drop_na(stat) %>% pull(gene_name) %>% as.character()
# ipsc_mn_sod1_datasets.intronList = sort(ipsc_mn_sod1_datasets.intronList, decreasing = TRUE)
# 
# # DNA damage response
# dna.damage_genes.list <- list("dna.damage" = ipsc_mn_sod1_datasets$irfinder$gene_name[ipsc_mn_sod1_datasets$irfinder$gene_name %in% gprofiler_database$`DNA Damage Response`])
# ipsc_mn_sod1_datasets.dna.damage_fgseaRes <- fgsea(pathways = dna.damage_genes.list, stats = ipsc_mn_sod1_datasets.intronList)
# ipsc_mn_sod1_datasets.dna.damage_fgseaRes
# # pathway pval padj log2err        ES NES size                             leadingEdge
# # 1: dna.damage   NA   NA      NA 0.6121127  NA   64 PRKDC,PIDD1,CDKN1A,CDC25A,ATR,MRE11,...
# ipsc_mn_sod1_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_sod1_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(DNA~damage~response)) + xlab("") +
#   annotate("text", x = 180000, y = 0.5, label = paste0("NES = ", round(ipsc_mn_sod1_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_sod1_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_sod1_datasets.dna.damage.gseaplot
# 
# # p53 signalling pathway
# p53.signalling_genes.list <- list("p53.signalling" = ipsc_mn_sod1_datasets$irfinder$gene_name[ipsc_mn_sod1_datasets$irfinder$gene_name %in% p53.signalling.pathway])
# ipsc_mn_sod1_datasets.p53.signalling_fgseaRes <- fgsea(pathways = p53.signalling_genes.list, stats = ipsc_mn_sod1_datasets.intronList)
# ipsc_mn_sod1_datasets.p53.signalling_fgseaRes
# # pathway pval padj log2err        ES NES size                               leadingEdge
# # 1: p53.signalling   NA   NA      NA 0.5981479  NA   93 MUC1,CDK5RAP3,CHD5,EHMT2,CDKN1A,HIF1A,...
# ipsc_mn_sod1_datasets.p53.signalling.gseaplot = plotEnrichment(p53.signalling_genes.list[["p53.signalling"]], ipsc_mn_sod1_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(p53~signalling)) + xlab("") +
#   annotate("text", x = 180000, y = 0.6, label = paste0("NES = ", round(ipsc_mn_sod1_datasets.p53.signalling_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_sod1_datasets.p53.signalling_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_sod1_datasets.p53.signalling.gseaplot
# 
# 
# ## GSEA tardbp: DNA damage response, p53 signalling pathway
# ipsc_mn_tardbp_datasets.intronList <- ipsc_mn_tardbp_datasets$irfinder %>% drop_na(stat) %>% pull(stat)
# names(ipsc_mn_tardbp_datasets.intronList) <- ipsc_mn_tardbp_datasets$irfinder %>% drop_na(stat) %>% pull(gene_name) %>% as.character()
# ipsc_mn_tardbp_datasets.intronList = sort(ipsc_mn_tardbp_datasets.intronList, decreasing = TRUE)
# 
# # DNA damage response
# dna.damage_genes.list <- list("dna.damage" = ipsc_mn_tardbp_datasets$irfinder$gene_name[ipsc_mn_tardbp_datasets$irfinder$gene_name %in% gprofiler_database$`DNA Damage Response`])
# ipsc_mn_tardbp_datasets.dna.damage_fgseaRes <- fgsea(pathways = dna.damage_genes.list, stats = ipsc_mn_tardbp_datasets.intronList)
# ipsc_mn_tardbp_datasets.dna.damage_fgseaRes
# # pathway pval padj log2err        ES NES size                        leadingEdge
# # 1: dna.damage   NA   NA      NA 0.4157656  NA   64 ATR,RFC1,BBC3,APAF1,BRCA1,TP53,...
# ipsc_mn_tardbp_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_tardbp_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(DNA~damage~response)) + xlab("") +
#   annotate("text", x = 180000, y = 0.5, label = paste0("NES = ", round(ipsc_mn_tardbp_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_tardbp_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_tardbp_datasets.dna.damage.gseaplot
# 
# # p53 signalling pathway
# p53.signalling_genes.list <- list("p53.signalling" = ipsc_mn_tardbp_datasets$irfinder$gene_name[ipsc_mn_tardbp_datasets$irfinder$gene_name %in% p53.signalling.pathway])
# ipsc_mn_tardbp_datasets.p53.signalling_fgseaRes <- fgsea(pathways = p53.signalling_genes.list, stats = ipsc_mn_tardbp_datasets.intronList)
# ipsc_mn_tardbp_datasets.p53.signalling_fgseaRes
# # pathway pval padj log2err        ES NES size                          leadingEdge
# # 1: p53.signalling   NA   NA      NA 0.4522344  NA   92 EHMT2,ATR,CHD5,NTRK3,BLM,ZNF385A,...
# ipsc_mn_tardbp_datasets.p53.signalling.gseaplot = plotEnrichment(p53.signalling_genes.list[["p53.signalling"]], ipsc_mn_tardbp_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(p53~signalling)) + xlab("") +
#   annotate("text", x = 180000, y = 0.6, label = paste0("NES = ", round(ipsc_mn_tardbp_datasets.p53.signalling_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_tardbp_datasets.p53.signalling_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_tardbp_datasets.p53.signalling.gseaplot
# 
# 
# ## GSEA fus: DNA damage response, p53 signalling pathway
# ipsc_mn_fus_datasets.intronList <- ipsc_mn_fus_datasets$irfinder %>% drop_na(stat) %>% pull(stat)
# names(ipsc_mn_fus_datasets.intronList) <- ipsc_mn_fus_datasets$irfinder %>% drop_na(stat) %>% pull(gene_name) %>% as.character()
# ipsc_mn_fus_datasets.intronList = sort(ipsc_mn_fus_datasets.intronList, decreasing = TRUE)
# 
# # DNA damage response
# dna.damage_genes.list <- list("dna.damage" = ipsc_mn_fus_datasets$irfinder$gene_name[ipsc_mn_fus_datasets$irfinder$gene_name %in% gprofiler_database$`DNA Damage Response`])
# ipsc_mn_fus_datasets.dna.damage_fgseaRes <- fgsea(pathways = dna.damage_genes.list, stats = ipsc_mn_fus_datasets.intronList)
# ipsc_mn_fus_datasets.dna.damage_fgseaRes
# # pathway pval padj log2err        ES NES size                        leadingEdge
# # 1: dna.damage   NA   NA      NA 0.1394417  NA   64 ATR,CCNB3,RFC1,PIDD1,MDM2,CDK6,...
# ipsc_mn_fus_datasets.dna.damage.gseaplot = plotEnrichment(dna.damage_genes.list[["dna.damage"]], ipsc_mn_fus_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(DNA~damage~response)) + xlab("") +
#   annotate("text", x = 180000, y = 0.5, label = paste0("NES = ", round(ipsc_mn_fus_datasets.dna.damage_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_fus_datasets.dna.damage_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_fus_datasets.dna.damage.gseaplot
# 
# # p53 signalling pathway
# p53.signalling_genes.list <- list("p53.signalling" = ipsc_mn_fus_datasets$irfinder$gene_name[ipsc_mn_fus_datasets$irfinder$gene_name %in% p53.signalling.pathway])
# ipsc_mn_fus_datasets.p53.signalling_fgseaRes <- fgsea(pathways = p53.signalling_genes.list, stats = ipsc_mn_fus_datasets.intronList)
# ipsc_mn_fus_datasets.p53.signalling_fgseaRes
# #  pathway pval padj log2err        ES NES size                           leadingEdge
# # 1: p53.signalling   NA   NA      NA 0.2007774  NA   93 CHD5,BANP,SPRED2,CHD8,TP53BP1,ATR,...
# ipsc_mn_fus_datasets.p53.signalling.gseaplot = plotEnrichment(p53.signalling_genes.list[["p53.signalling"]], ipsc_mn_fus_datasets.intronList) + 
#   theme(axis.text=element_text(size=8), axis.title=element_text(size=10)) + ylab(expression(p53~signalling)) + xlab("") +
#   annotate("text", x = 180000, y = 0.6, label = paste0("NES = ", round(ipsc_mn_fus_datasets.p53.signalling_fgseaRes$NES, digits = 3),"\nP = ", round(ipsc_mn_fus_datasets.p53.signalling_fgseaRes$pval, digits = 3)), size = 2.8, hjust = 0) + theme_oz()
# ipsc_mn_fus_datasets.p53.signalling.gseaplot
...
compare_mutations_irfinder_volcano_multiplot <- plot_grid(
  plot_grid(ipsc_mn_sporadic_datasets.ir.volcano, ipsc_mn_fus_datasets.ir.volcano, ipsc_mn_tardbp_datasets.ir.volcano, ipsc_mn_c9orf72_datasets.ir.volcano, ipsc_mn_sod1_datasets.ir.volcano, ipsc_mn_mutations_irfinder_ggheatmap, labels = "auto", ncol = 3, nrow = 2),
  ipsc_mn_mutants_bind_datasets.irfinder.upset_intron_level, 
  plot_grid(ipsc_mn_sporadic_datasets.ir.go_curated, ipsc_mn_fus_datasets.ir.go_curated,ipsc_mn_tardbp_datasets.ir.go_curated, labels = c("h","i","j"), ncol = 3),
  labels = c("","g"), nrow = 3, rel_heights = c(3,1,1.5)) 
# compare_mutations_irfinder_volcano_multiplot
ggsave(compare_mutations_irfinder_volcano_multiplot, filename = here(proj_path,"splicing/irfinder/compare_mutations_irfinder_volcano_multiplot.png"), height = 13.5, width = 10)
...
# 168 IR events from nature comms

```{r}
ipsc_mn_fus_datasets = readRDS(here(proj_path,"expression/deseq2/ipsc_mn_fus_datasets.rds")) # 84M
ipsc_mn_sod1_datasets = readRDS(here(proj_path,"expression/deseq2/ipsc_mn_sod1_datasets.rds")) # 162M

nat.comms.168.selection_events = read_csv(here(camp_path,"projects/luisier-nature-comms-2018-sfpq-ir/splicing/raphaelle_vast_tools/168genes_VASTTools_selection_events.csv")) %>% clean_names() %>%
  mutate(coords = gsub("chr","",coord), Chr = str_split_fixed(coords,":",2)[,1], Start = str_split_fixed(coords,"-",2)[,1], Start = as.numeric(str_split_fixed(Start,":",2)[,2]), End = as.numeric(str_split_fixed(coords,"-",2)[,2])) %>%
  select(gene_name = gene, Chr, Start, End)
nat.comms.168.selection_events

nat.comms.168.selection_events %>% distinct()
nat.comms.168.selection_events %>% get_dupes()

# SOD1
ipsc_mn_sod1_datasets.ir.nat.comms.168 = nat.comms.168.selection_events %>% left_join(ipsc_mn_sod1_datasets$irfinder)
ipsc_mn_sod1_datasets.irfinder.filt = ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% filter(log2FoldChange != 0) %>% mutate(condition = "Intron retention\nALS vs CTRL")
ipsc_mn_sod1_datasets.ir.violin <- violin_plot(ipsc_mn_sod1_datasets.irfinder.filt, color_by = "condition", cols = "firebrick2", one_sample = TRUE, stats_test = "t_test", arrow_label_x = 0.87, numerator = "ALS", denomenator = "CTRL", ylims = c(-1.5,1.5), dpi = 300)
ipsc_mn_sod1_datasets.ir.violin
# [1] "Using one sample t_test"
# # A tibble: 1 × 9
#   group       .y.   group1 group2          n statistic     df     p p.signif
#   <chr>       <chr> <chr>  <chr>       <int>     <dbl>  <dbl> <dbl> <chr>   
...
# # Density
# ipsc_mn_sod1_datasets.ir.density <- density_plot(ipsc_mn_sod1_datasets.irfinder.filt, color_by = "condition", cols = "forestgreen",  xlab_prefix = "Intron Retention", numerator = "ALS", denomenator = "CTRL", xmax = 1.5, dpi = 300)
# ipsc_mn_sod1_datasets.ir.density

## Volcano
ipsc_mn_sod1_datasets.ir.als.labels <- ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% filter(abs(stat) > 5, gene_name %in% c(ALS_genes)) %>% arrange(-abs(stat)) %>% distinct(gene_name, .keep_all = TRUE) %>% top_n(8, wt = abs(stat))  %>% pull(gene_name)
ipsc_mn_sod1_datasets.ir.p53.labels <- ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% filter(abs(stat) > 5, log2FoldChange > 0.5, gene_name %in% c(p53.signalling.pathway)) %>% arrange(-abs(stat)) %>% distinct(gene_name, .keep_all = TRUE) %>% top_n(8, wt = abs(stat)) %>% pull(gene_name)
ipsc_mn_sod1_datasets.ir.synapse.labels <- ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% filter(padj < 0.05, gene_name %in% c(gprofiler_database$synapse)) %>% arrange(-abs(stat)) %>% distinct(gene_name, .keep_all = TRUE) %>% filter(stat < 0) %>% top_n(8, wt = abs(stat)) %>% pull(gene_name)
ipsc_mn_sod1_datasets.ir.als.labels.filt = c("UNC13A","STMN2","SETX","HNRNPA1","ATXN2","C9orf72","TARDBP")# ,"TIA1","VAPB","CHMP2B",,"UNC13B"", "QKI")     
# ipsc_mn_sod1_datasets.ir.rbp.labels <- ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% filter(padj < 0.0001, gene_name %in% c(ALS_genes, RNA_BINDING_PROTEINS)) %>% distinct(gene_name) %>% pull(gene_name)
ipsc_mn_sod1_datasets.ir.volcano <- volcano_plot(ipsc_mn_sod1_datasets.ir.nat.comms.168, numerator = "ALS", denomenator = "CTRL",
               title = "Intron retention: pan ALS", subtitle = "101 Controls vs 318 ALS", ymax = 17, xmax = 2, dpi = 300,
               gene_labels = c(ipsc_mn_sod1_datasets.ir.als.labels, ipsc_mn_sod1_datasets.ir.als.labels.filt, ipsc_mn_sod1_datasets.ir.p53.labels, "HDAC1", "CFAP410", ipsc_mn_sod1_datasets.ir.synapse.labels))
# ipsc_mn_sod1_datasets.ir.volcano
...
ipsc_mn_sod1_datasets.ir.168.nat.comms = ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% filter(coords %in% nat.comms.168.selection_events$coords)

ipsc_mn_sod1_datasets.ir.nat.comms.168 %>% glimpse
