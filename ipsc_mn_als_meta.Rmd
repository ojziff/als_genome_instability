...
credentials::set_github_pat("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") 
# make github
library(gert)
...
# Bradley_FB_ipsc   = "#E31A1C")) +
  theme_bw() + theme(panel.grid = element_blank(), legend.text=element_text(size=8), legend.title=element_text(size=10), legend.position = "right", legend.spacing.y = unit(0.05, 'cm')) + 
...
postmitotic.motor.neuron <- c("CHAT","MNX1","ISL1","FOXP1","LHX3","ETV4","LMO4","NOTCH1")
mn.markers = c(postmitotic.motor.neuron,"SYT1","MAP2","STMN2","NEFM","SNAP25","SYT4","NCAM1","SCG2","GAP43","L1CAM","TUBB3","NEFL")
interneuron.markers <- c("LHX1","LHX5","DRGX","LMX1B","POU4F1","LBX1","EVX1","EN1","SIM1")
...
neural.crest <- c("SOX10","NGFR","FOXD3","SNAI1","SNAI2","LMX1A")  # ,"GDF7" roof plate          
...
differentiation.markers.rnaseq.heatmap <- Heatmap(differentiation.markers.marker.mat,
          border = TRUE, heatmap_legend_param = list(title = "", direction = "horizontal"),
          cluster_rows = FALSE, cluster_row_slices = FALSE, cluster_columns = FALSE,
...
# HOX.markers <- c("HOXA1", "HOXA3", "HOXA4", "HOXA5","HOXA6", "HOXA7", "HOXA9", "HOXA10", "HOXA11", "HOXA13", # HOX1
#                  "HOXB1","HOXB5","HOXB9","HOXB6","HOXB7","HOXB2","HOXB3","HOXB4","HOXB8", #HOX2
#                  "HOXC9","HOXC5","HOXC13","HOXC12","HOXC4","HOXC6","HOXC8", # HOX3
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX4
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX5
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX6
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX7
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX8
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX9
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX10
#                  "HOXD1","HOXD3","HOXD11","HOXD4","HOXD8","HOXD9","HOXD10", # HOX11
#                  "HOXB4", "HOXA5", "HOXC5", "HOXC6", "HOXC8", "HOXC9", "HOXD10","TLX1","HOXD12","HOXA13")

HOX.markers <- gtf %>% filter(grepl("^HOX[A-D][0-9]+", gene_name)) %>% distinct(gene_name) %>% pull(gene_name)

HOX.markers.df <- data.frame(row.names = HOX.markers, gene_name = factor(HOX.markers, levels = HOX.markers)) 
...
HOX.markers.rnaseq.heatmap <- Heatmap(HOX.markers.marker.mat, 
          ...
          # row_split = factor(differentiation.markers.df.filt$group, levels = unique(differentiation.markers.df.filt$group)),
          # right_annotation = rowAnnotation(markers = anno_block(gp = gpar(fill = rep("white",4)), labels = c("Hindbrain", "Cervical", "Thoracic", "Lumbar"), labels_gp = gpar(fontsize = 8), labels_rot = 0)),
...
```{r}
png("/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/figS2.heatmap.cell.type.markers.png",width=8.25,height=11.75,units="in",res=800, type="cairo")
draw(celltype.markers.rnaseq.heatmap, heatmap_legend_side = "top")
dev.off()

figS3.merged <- differentiation.markers.rnaseq.heatmap %v% HOX.markers.rnaseq.heatmap 
png("/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/figS3.heatmaps.differentiation.hox.merged.png",width=8.25,height=11.75,units="in",res=800, type="cairo")
draw(figS3.merged, heatmap_legend_side = "top")
dev.off()
...
volcano_plot <- function(de_res, title = NULL, subtitle = NULL, annotate_by = NULL, ymax = 16.5, xlim = c(-3,3), xpos = 0.5){
  ...
  plot <- de_res %>% mutate( pvalue = ifelse( pvalue < 1e-16, Inf, pvalue)) %>% #threshold at 1e16
    ...
    ggrastr::rasterise(geom_point(aes(colour = class ), size = 0.5), dpi = 300) +
...
ipsc_mn_mutants_bind_datasets.res.upset = ipsc_mn_mutants_bind_datasets.res %>% filter(padj < 0.05) %>% select(mutation, gene_name) %>% group_by(gene_name) %>% summarise(mutations = list(mutation)) %>%
  ggplot(aes(x = mutations)) + geom_bar() +
    ...
    scale_y_continuous(breaks = NULL, lim = c(0, 2600), name = "") +
...
ipsc_mn_mutants_bind_datasets.res.upset = ipsc_mn_mutants_bind_datasets.dir.res %>% filter(padj < 0.05) %>% select(mutation, gene_name) %>% group_by(gene_name) %>% summarise(mutations = list(mutation)) %>%
  ggplot(aes(x = mutations)) + geom_bar() +
      geom_text(stat='count', aes(label=after_stat(count)), vjust=-1, size = 2.8) +
    scale_x_upset(order_by = "degree")  + # n_intersections = 20
...
# SOD1 and TARDBP show many opposing changes
```


Fisher test overlap mutations
...
# Table S2. DEG overlap in iPSN ALS datasets

```{r}
ipsc_mn_mutants_join_datasets.res = mutate(select(ipsc_mn_c9orf72_datasets$res, gene_name, gene_id, padj.c9orf72 = padj, lfc.c9orf72 = log2FoldChange), direction.c9orf72 = case_when(padj.c9orf72 < 0.05 & lfc.c9orf72 > 0 ~ "up", padj.c9orf72 < 0.05 & lfc.c9orf72 < 0 ~ "down", TRUE ~ "non-significant")) %>%
  full_join(mutate(select(ipsc_mn_fus_datasets$res, gene_name, gene_id, padj.fus = padj, lfc.fus = log2FoldChange), direction.fus = case_when(padj.fus < 0.05 & lfc.fus > 0 ~ "up", padj.fus < 0.05 & lfc.fus < 0 ~ "down", TRUE ~ "non-significant"))) %>%
  full_join(mutate(select(ipsc_mn_tardbp_datasets$res, gene_name, gene_id,  padj.tardbp = padj, lfc.tardbp = log2FoldChange), direction.tardbp = case_when(padj.tardbp < 0.05 & lfc.tardbp > 0 ~ "up", padj.tardbp < 0.05 & lfc.tardbp < 0 ~ "down", TRUE ~ "non-significant"))) %>%
  full_join(mutate(select(ipsc_mn_sod1_datasets$res, gene_name, gene_id, padj.sod1 = padj, lfc.sod1 = log2FoldChange), direction.sod1 = case_when(padj.sod1 < 0.05 & lfc.sod1 > 0 ~ "up", padj.sod1 < 0.05 & lfc.sod1 < 0 ~ "down", TRUE ~ "non-significant"))) %>%
  full_join(mutate(select(ipsc_mn_vcp_datasets$res, gene_name, gene_id, padj.vcp = padj, lfc.vcp = log2FoldChange), direction.vcp = case_when(padj.vcp < 0.05 & lfc.vcp > 0 ~ "up", padj.vcp < 0.05 & lfc.vcp < 0 ~ "down", TRUE ~ "non-significant"))) %>%
  # arrange() %>%
  select(gene_name, gene_id, everything())

write_csv(ipsc_mn_mutants_join_datasets.res, "/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/TableS3.overlap_als_individual_datasets.csv")
...
# Fig S3-4 Gene expression mutations scatters
...
```{r}
# c9orf72
ipsc_mn_c9orf72_datasets.rrvgo = gprofiler2rrvgo(ipsc_mn_c9orf72_datasets$res , query1 = "Down", query2 = "Up", GO_cat = "BP", sig.filter = "padj", species = "human")
ipsc_mn_c9orf72_datasets.rrvgo =  ipsc_mn_c9orf72_datasets.rrvgo %>% mutate(term = gsub("homophilic cell adhesion via plasma membrane adhesion molecules", "plasma membrane adhesion", term), parentTerm = term)
...
ipsc_mn_sod1_datasets.rrvgo =  ipsc_mn_sod1_datasets.rrvgo %>%  mutate(term = gsub("regulation of ", "", term), parentTerm = term)
...
# vcp
# ipsc_mn_vcp_datasets.rrvgo = gprofiler2rrvgo(filter(ipsc_mn_vcp_datasets$res, log2FoldChange < 0), query1 = "Down", query2 = NULL, GO_cat = "BP", sig.filter = "padj", species = "human")
...
ggsave(de_go_terms_mutations_merged, filename = "/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/figS5.de_go_terms_mutations_merged.png", height = 11.75, width = 8.25)
...
hallmark_gsea_plot <- function(df, plot_all = FALSE){
  ...
  hallmark_levels <- 
    ...
    rowwise() %>%
    mutate(FUS = NA, VCP = NA, # not enriched for any pathway
           mean_nes = mean( c(C9orf72, TARDBP, FUS, SOD1), na.rm = TRUE)) %>%
    arrange( mean_nes) %>%
...
h_plot_all
ggsave(h_plot_all, filename = "/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/figS6.hallmarks_gsea_plot_all.png", height = 11.75, width = 8.25)
...
```{r}
ipsc_mn_als_datasets.als.gene.labels <- ipsc_mn_als_datasets$res %>% filter(padj < 0.05, gene_name %in% c(ALS_genes, RNA_BINDING_PROTEINS)) %>% pull(gene_name)
ipsc_mn_als_datasets.volcano <- volcano_plot(ipsc_mn_als_datasets$res, "panALS", 
               subtitle = "47 Controls vs 52 ALS", ymax = 10,
               annotate_by = ipsc_mn_als_datasets.als.gene.labels) 
ipsc_mn_als_datasets.volcano

ipsc_mn_als_datasets$res %>% filter(padj<0.05, log2FoldChange > 0) # 193
ipsc_mn_als_datasets$res %>% filter(padj<0.05, log2FoldChange < 0) # 821
...
```{r}
ipsc_mn_panals_list = list("panALS" = ipsc_mn_als_datasets$res)
panals <- names(ipsc_mn_panals_list)

ipsc_mn_panals_list = purrr::map2(
      .x = ipsc_mn_panals_list,
      .y = names(ipsc_mn_panals_list), ~{
        .x$mutation <- .y
        return(.x)
      })
...
ipsc_mn_panals_gene_lists = make_gene_sets(ipsc_mn_panals_list)
...
ipsc_mn_panals_hallmark_gsea = hallmark_gsea(ipsc_mn_panals_gene_lists$lfc)
...
hallmark_gsea_plot <- function(df, plot_all = FALSE){
  ...
  hallmark_levels <- 
    ...
    pivot_wider(names_from = set, values_from = NES) %>%
    rowwise() %>%
    mutate(mean_nes = mean( panALS, na.rm = TRUE)) %>%
    arrange( mean_nes) %>%
    mutate(ID = factor(ID, levels = ID)) 
...
h_plot <- hallmark_gsea_plot(ipsc_mn_panals_hallmark_gsea)
h_plot 

ipsc_mn_panals_hallmark_gsea_h_plot_all <- hallmark_gsea_plot(ipsc_mn_panals_hallmark_gsea, plot_all = TRUE)
ipsc_mn_panals_hallmark_gsea_h_plot_all
ggsave(ipsc_mn_panals_hallmark_gsea_h_plot_all, filename = "/camp/home/ziffo/home/projects/ipsc-mn-als-meta/expression/deseq2/fig2.hallmarks_gsea_plot_all.png", height = 11.75, width = 8.25)
